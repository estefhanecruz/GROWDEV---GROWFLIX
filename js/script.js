...
function renderVideos() {
	// 1. Mapeamos quais IDs do HTML correspondem a quais categorias
	const sections = {
	...
	// 2. Limpamos as seções antes de renderizar (boa prática)
	// 3. Usamos o .forEach para percorrer nosso array 'movies' (Módulo 4!)
	movies.forEach((movie) => {
		const container = sections[movie.category];

		if (container) {
			// Criamos o HTML do Card (usando as classes do Bootstrap que você já domina)
			const cardHTML = `
                <div class="col-12 col-md-4 col-lg-3 card-movie" onclick="openMovie('${movie.link}')">
                    <img src="${movie.img}" alt="${movie.title}" class="img-fluid rounded shadow">
                    <p class="text-light mt-2">${movie.title}</p>
                </div>
            `;
			// Injetamos no HTML (Módulo 5)
			container.innerHTML += cardHTML;
		}
	});
}

// Chamamos a função assim que a página carrega
renderVideos();

// Modal
function openMovie(videoUrl) {
	const videoIframe = document.getElementById("videoIframe");
	const movieModal = new bootstrap.Modal(document.getElementById("movieModal"));

	// 1. Coloca o link do vídeo no iframe (Adicionando o autoplay para facilitar)
	videoIframe.src = videoUrl + "?autoplay=1";

	// 2. Abre o modal
	movieModal.show();

	// 3. Importante: Limpar o link quando o modal fechar para o vídeo parar de tocar
	document
